...
#' @param digit an integer corresponding to the digit to be extracted.
...
extract_digit <- function(x, digit, number.of.digits=1){
  x <- abs(x)
  as.integer(as.numeric(substr(x, nchar(x)-(nchar(x)-digit), nchar(x)-(nchar(x)-number.of.digits))))
}
